...
def _prepare_effective_frames(question: dict[str, Any], data_root: Path) -> tuple[str, list[str], float, dict[str, Any], list[str], list[str]]:
    ...
    prefixes = _prefixes_for_effective_frames(len(frames), EGO_SURGERY_FPS, cleaned, options, window_meta)
    return cleaned, options, EGO_SURGERY_FPS, window_meta, frames, prefixes
...
def _predict_ego(
    ...
    if qt == SPATIAL_QT:
        ...
        active_sample, _ = uniform_sample_frames(frames2, MAX_FRAMES_PER_CALL)
        sampled_offsets = list(active_sample.get("sampled_offsets", []))
    ...
    raise ValueError(f"Unsupported EgoSurgery question_type: {qt}")


def main() -> int:
    parser = argparse.ArgumentParser(description="Readable single-file EgoSurgery submission runner.")
    ...
    for idx, question in enumerate(rows, start=1):
        ...
        print(f"[{idx}/{len(rows)}] {question.get('id')} {question.get('question_type')} -> {answer}", flush=True)
